    ...
    public partial class Form1 : Form
    {
        ...
        private void DGV1()
        {
            if (ConnectionDB())
            {
                try
                {
                    string query = "SELECT * FROM fond.needy";
        ...
        private void SearchAll()
        {
            try
            {
                ...
                string query = "SELECT * FROM fond.needy WHERE 1=1";
        ...
        private void button2_Click(object sender, EventArgs e)
        {
            if (!string.IsNullOrEmpty(textBox1.Text) && !string.IsNullOrEmpty(textBox2.Text) && !string.IsNullOrEmpty(textBox3.Text) && !string.IsNullOrEmpty(maskedTextBox1.Text) && !string.IsNullOrEmpty(textBox5.Text))
            {
                try
                {
                    msCommand = new MySqlCommand("INSERT INTO fond.needy(name, surname, patronymic, phone_number, email) " +
        ...
        private void DisplayData()
        {
            myconnect.Open();
            DataTable dt = new DataTable();
            msDataAdapter = new MySqlDataAdapter("select * from fond.needy", myconnect);
        ...
        private void button6_Click(object sender, EventArgs e)
        {
            ...
            dataGridView1.SelectedRows.Count > 0)
            {
                try
                {
                    ...
                    int idNeedy = Convert.ToInt32(dataGridView1.SelectedRows[0].Cells["idNeedy"].Value);
                    ...
                    MySqlCommand msCommand = new MySqlCommand("UPDATE fond.needy SET name=@name, surname=@surname, patronymic=@patronymic, phone_number=@phone_number, email=@email WHERE idNeedy=@idNeedy", myconnect);
                    ...
                    msCommand.Parameters.AddWithValue("@phone_number", maskedTextBox1.Text);
                    msCommand.Parameters.AddWithValue("@email", textBox5.Text);
                    msCommand.Parameters.AddWithValue("@idNeedy", idNeedy);
        ...
        private void button7_Click(object sender, EventArgs e)
        {
            if (dataGridView1.SelectedRows.Count > 0)
            {
                try
                {
                    ...
                    MySqlCommand msCommand = new MySqlCommand("DELETE FROM fond.needy WHERE idNeedy=@idNeedy", myconnect);
        ...
        private void DisplayDistinctData()
        {
            try
            {
                if (ConnectionDB())
                {
                    string query = "SELECT DISTINCT idNeedy, name, surname, patronymic, phone_number, email, birth_date FROM fond.needy";
                    MySqlDataAdapter mySqlDataAdapter = new MySqlDataAdapter(query, myconnect);
                    DataTable dt = new DataTable();
                    mySqlDataAdapter.Fill(dt);
                    dataGridView1.DataSource = dt;

                    // Обновление заголовков и порядка столбцов
                    ColumnName1();
                }
            }
            catch (Exception ex)
        ...
        {
            if (comboBox1.SelectedItem.ToString() == "без повторений")
            {
                DisplayDistinctData();
            }

            if (comboBox1.SelectedItem.ToString() == "первые 10 записей")
            {
                DisplayFirstTenRecords();
            }

            if (comboBox1.SelectedItem.ToString() == "последние 15 записей")
            {
                DisplayLastFifteenRecords();
            }
        }
        private void DisplayFirstTenRecords()
        {
            try
            {
                if (ConnectionDB())
                {
                    string query = "SELECT * FROM fond.needy LIMIT 10";
                    MySqlDataAdapter mySqlDataAdapter = new MySqlDataAdapter(query, myconnect);
                    DataTable filteredTable = new DataTable();
                    mySqlDataAdapter.Fill(filteredTable);
        ...
        private void DisplayLastFifteenRecords()
        {
            try
            {
                if (ConnectionDB())
                {
                    // Измените 'idNeedy' на нужное имя столбца, если необходимо
                    string query = "SELECT * FROM fond.needy ORDER BY idNeedy DESC LIMIT 15";
                    MySqlDataAdapter mySqlDataAdapter = new MySqlDataAdapter(query, myconnect);
                    DataTable filteredTable = new DataTable();
                    mySqlDataAdapter.Fill(filteredTable);
                    dataGridView1.DataSource = filteredTable;
        ...
        {
            ShowAnniversariesNextMonth();
        }
        private void ShowAnniversariesNextMonth()
        {
            if (ConnectionDB())
            {
                try
                {
                    string query = @"
                        SELECT 
                            ...
                            DATE_FORMAT(
                                ADDDATE(birth_date, INTERVAL YEAR(CURDATE()) - YEAR(birth_date) +
                                IF(
                                    DATE_FORMAT(CURDATE(), '%m-%d') <= DATE_FORMAT(birth_date, '%m-%d'), 
                                    0, 
                                    1
                        ...
                        FROM 
                            fond.needy
                        WHERE 
                            MONTH(birth_date) = MONTH(CURDATE() + INTERVAL 1 MONTH)
                            AND (
                                YEAR(CURDATE()) - YEAR(birth_date) + 
                                IF(
                                    DATE_FORMAT(CURDATE(), '%m-%d') <= DATE_FORMAT(birth_date, '%m-%d'), 
